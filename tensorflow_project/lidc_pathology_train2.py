...
net_store_name = "lidc_3D_multi_crop_net_64_aug_test2"
net_store_path = store_path + "/" + net_store_name
tensorboard_path = net_store_path + "/tensorboard/"
#net_init_name = "lidc_3D_multi_crop_net_64_aug_test"
...
if 'net_init_name' in dir():
	...
	lidc_dir="../data_samples/lidc_cubes_64_overbound"
	files_lists, _ = lt.filelist_training(lidc_dir, shuffle=True, cross_fold=5, val_fold=4, test_fold=5)
...
net_outs, _ = tmc.multi_crop_net(volume_reshape, poolings=[1,1,2], weight_decay_coefficiency=0.0005)
loss = tf.nn.softmax_cross_entropy_with_logits_v2(logits = net_outs['last_out'], labels = real_label)
		...
		for epoch in range(epochs[train_stage]):
			...
			for bi in tqdm(range(0, train_num, DATA_BATCH_SIZE)):
				...
				for ti in range(bi, batchend):
					...
					malignancy = lt.sample_malignancy_label(file)
					if malignancy==1:
						label = [1.0, 0]
