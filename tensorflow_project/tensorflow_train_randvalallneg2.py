...
net_store_name = "luna_3D_multi_crop_net_56_aug2"
net_store_path = store_path + "/" + net_store_name
tensorboard_path = net_store_path + "/tensorboard/"
#net_init_name = "luna_3D_multi_crop_net_56_aug"
...
if 'net_init_name' in dir():
	...
	net_init_file = net_init_path + "/epoch20/epoch20"
...
net_outs, _ = tmc.multi_crop_net(volume_reshape, channels=[[32,32,32],[64,2]], poolings=[1,1,1], positive_confidence=positive_confidence)
...
extract_volumes = ft.partial(da.extract_volumes, volume_shape=np.int_([REGION_SIZE, REGION_SIZE, REGION_SIZE]), centering=CENTERING, scale_augment=SCALE_AUGMENTATION, translation_augment=TRANSLATION_AUGMENTATION, rotation_augment=ROTATION_AUGMENTATION, flip_augment=FLIP_AUGMENTATION)
		...
		for epoch in range(epochs[train_stage]):
			...
			for pbi in tqdm(range(0, positive_train_num, positive_batch_size)):
				...
				for pti in range(pbi, posbatchend):
					...
					if pfile.split('/')[0].find("luna")>=0:
						patient_uid, nodule_diameter = mt.get_luna_annotation_informations(pfile, "luna_cubes_56_overbound/luna_annotations.csv")
					elif pfile.split('/')[0].find("tianchi")>=0:
						patient_uid, nodule_diameter = mt.get_luna_annotation_informations(pfile, "tianchi_cubes_56_overbound/tianchi_annotations.csv")
					else:
					...
					if "positive_batch" not in dir():
						...
						#positive_batch = np.concatenate((positive_batch, extract_volumes(positive_data, nodule_diameter=0)), axis=0)
					del positive_data
			...
			for pbi in tqdm(range(0, positive_val_num, positive_batch_size)):
				...
				for pvi in range(pbi, posbatchend):
					...
					if pfile.split('/')[0].find("luna") >= 0:
						patient_uid, nodule_diameter = mt.get_luna_annotation_informations(pfile,
				...
				for bi in range(0, val_data.shape[0], BATCH_SIZE):
					...
					if 'bn_params' in dir():
						losses, accuracies, bn_stat, val_summary = sess.run([loss, correct_prediction, bn_params, val_merge], {volume_input: data_batch, real_label: label_batch})
					else:
			...
			if 'bn_stats' in dir() and batch_num > 0:
				bn_stats /= batch_num
			if pvd_num > 0:
				pvd_loss /= pvd_num
