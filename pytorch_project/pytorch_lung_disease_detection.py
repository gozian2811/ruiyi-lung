...
def detection(**kwargs):
	...
	if hasattr(opt, 'filelists') and 'test' in opt.filelists.keys():
		...
		pd.DataFrame(data=test_uids, columns=['series_uid']).to_csv(evaluation_path+'/patients_uid.csv', index=False)
	#else:
	...
	for p in range(len(test_patients)):
		...
		'''
		if 'annotation_file' in dir() and len(nodule_center_predictions)<1000:
...
def detection_fusion(test_path=None, result_path="./experiments_dt/evaluations_tianchild_densecropnet_fusion_B_3", **kwargs):
	opt.parse(kwargs)
	if test_path is None:
		test_paths = ["/data/fyl/datasets/Tianchi_Lung_Disease/testB"]
	else:
		test_paths = [test_path]
	#test_sample_filelist = "/data/fyl/models_pytorch/DensecropNet_detection_test_rfold1/filelist_val_fold0.log"
	net_files = ["/data/fyl/models_pytorch/DensecropNet_nodule_detection_2_rfold1/DensecropNet_nodule_detection_2_rfold1_epoch1",
		     "/data/fyl/models_pytorch/DensecropNet_stripe_detection_2_rfold1/DensecropNet_stripe_detection_2_rfold1_epoch3"]
	#annotation_file = "/data/fyl/datasets/Tianchi_Lung_Disease/chestCT_round1_annotation.csv"
	#candidate_file = "/data/fyl/datasets/Tianchi_Lung_Disease/candidate.csv"
	labels = [1, 5]
	...
	for p in range(len(test_patients)):
		...
		candidate_results = nd.slic_candidate(image, 100, focus_area='lung')
		if candidate_results is None:
			continue
		candidate_coords, candidate_labels, cluster_labels = candidate_results
		if 'vision_path' in dir() and vision_path is not None:
		...
		print("result saved at {}" .format(result_path))
